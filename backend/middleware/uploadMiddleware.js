...
const fileFilter = (req, file, cb) => {
    ...
    const allowedVideoTypes = ['.mp4', '.mov', '.m4v', '.webm'];
    ...
    if (file.fieldname === 'image') {
        if (allowedImageTypes.includes(extname) && (!mimetype || mimetype.startsWith('image/') || mimetype === 'application/octet-stream')) {
            return cb(null, true);
        }
        cb(new Error('Only images (jpg, jpeg, png, webp, gif) are allowed!'));
    } else if (file.fieldname === 'video') {
        if (allowedVideoTypes.includes(extname) || mimetype.startsWith('video/') || mimetype === 'application/octet-stream') {
            return cb(null, true);
        }
        cb(new Error('Only videos (mp4, mov, m4v, webm) are allowed!'));
    } else if (file.fieldname === 'document') {
        if (allowedDocTypes.includes(extname) || mimetype.includes('pdf') || mimetype.includes('word') || mimetype.includes('document') || mimetype === 'application/octet-stream') {
            return cb(null, true);
        }
