...
const createPost = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (image && image.length > 0) {
            const storedAsset = await storeUploadedFile(image[0].path, {
                folder: "unilink/posts/images",
                resource_type: "auto"
            });
            postData.image = storedAsset.url;
            if (storedAsset.provider === 'cloudinary') cleanupFile(image[0].path);
        }

        // Handle Video Upload
        if (video && video.length > 0) {
            const storedAsset = await storeUploadedFile(video[0].path, {
                resource_type: "video",
                folder: "unilink/posts/videos"
            });
            postData.video = storedAsset.url;
            if (storedAsset.provider === 'cloudinary') cleanupFile(video[0].path);
        }

        // Handle Document Upload
        if (document && document.length > 0) {
            const storedAsset = await storeUploadedFile(document[0].path, {
                resource_type: "raw",
                folder: "unilink/posts/docs"
            });
            postData.document = storedAsset.url;
            if (storedAsset.provider === 'cloudinary') cleanupFile(document[0].path);
        }
